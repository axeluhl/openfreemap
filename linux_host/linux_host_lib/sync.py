...
def full_sync_local_versions() -> None:
    """local_versions mode: serve the newest runs already present locally, no downloads.

    For instances seeded from another host (golden AMI / copied runs): the upstream
    deployed-version endpoint is never contacted and no btrfs is downloaded. Each area's
    deployed pointer is set to the newest local run so nginx builds the latest/wildcard
    blocks for a version this instance actually holds.
    """
    with host_lock():
        try:
            download_assets()
        ...
        missing_areas = set(get_linux_host_config().areas) - local_run_versions.keys()
        if missing_areas:
            raise RuntimeError(f'no local runs found for: {", ".join(sorted(missing_areas))}')

        write_version_files(local_run_versions)
        active_versions = get_local_deployed_versions()
        retained_versions = {area: {version} for area, version in active_versions.items()}
