...
class FormatterOptions:
    """Class holds the formatting options."""
    indentation = 4
...
class Formatter:
    ...
    def format_string(self,
        ...
        max_empty = self.options.max_empty_lines
        for pattern, substitute in (
                (r'%s{%d,}' % (ls, max_empty + 2), ls * (max_empty + 1)),
                (r'^%s' % ls, ''),
                (r'%s$' % ls, '')
        ):
    ...
    def _load_file_content(self,
        ...
        :param file_path: Path to the original nginx configuration file."""
    ...
    def _strip_line(single_line):
        """Strips the line and replaces neighboring whitespaces with a single space (except when within quotation
        marks). """
    ...
    def _join_opening_bracket(lines):
        """When opening the curly bracket is in its own line (K&R convention), it's joined with the precluding line (Java)."""
        modified_lines = []
...
def _standalone_run(program_arguments):
    ...
    formatter_options_group.add_argument("--max-empty-lines",
                                         action="store",
                                         default=2,
                                         type=int,
                                         help="maximum number of consecutive empty lines to keep (default: 2)")
    ...
    format_options.max_empty_lines = args.max_empty_lines
