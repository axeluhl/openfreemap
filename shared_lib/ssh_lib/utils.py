...
def os_release_id(c: Connection) -> str:
    """Return the /etc/os-release ID (e.g. 'amzn' on Amazon Linux 2023)."""
...
def add_user(
    ...
    if system:
        c.sudo(f'useradd --system --shell /sbin/nologin --user-group {username}')
    else:
        # Amazon Linux 2023 ships the shadow-utils `useradd`, not Debian's
        # interactive `adduser`, so build the equivalent invocation. -m creates
        # the home dir; with no password set, login is ssh-key only.
        parts = ['useradd', '-m', '-s /bin/bash']
        if uid:
