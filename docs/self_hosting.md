...
1.  **Clone and prepare `config` folder**
    ...
    with the `--domain` switch on the command line (see the deploy step below), which takes precedence over
    `DOMAIN_DIRECT`.

    Set `SKIP_PLANET=true` first.

1.  **Set up Python if you don't have it yet**
...
1.  **Deploy quick version with `SKIP_PLANET=true`**
    ...
    *not* a separate positional argument — provide it either via your `~/.ssh/config`, via the `--user` option, or by
    ...
    For faster provisioning and fleet setups, see *Faster provisioning options* and
    *Baking a golden AMI* below (after the numbered steps).
...
1.  **Deploy and check with `SKIP_PLANET=false`**
