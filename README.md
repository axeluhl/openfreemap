...
2. OpenFreeMap is not something you can install locally. This repo is a deploy script specifically made to set up clean Amazon Linux 2023 servers or virtual machines (this fork; upstream targets Ubuntu 24.04). It uses [Fabric](https://www.fabfile.org/) and runs commands over SSH. With a single command it can set up a production-ready server, both the linux_host and tilegen components.
...
There is no cloud, just dedicated servers. The web server is nginx on Amazon Linux 2023 (this fork; upstream uses Ubuntu 24.04).
...
`linux_host/deploy_linux_host.py` and `tilegen/deploy_tilegen.py` set up clean Amazon Linux 2023 servers over SSH (this fork; upstream targets Ubuntu 24.04).
...
The license of this project is [MIT](https://www.tldrlegal.com/license/mit-license). Map data is from [OpenStreetMap](https://www.openstreetmap.org/copyright). The licenses for included projects are listed in [LICENSE.md](https://github.com/hyperknot/openfreemap/blob/AL2023/LICENSE.md).
